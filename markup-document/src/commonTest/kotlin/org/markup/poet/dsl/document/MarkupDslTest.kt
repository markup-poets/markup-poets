...
import org.markup.poet.dsl.table.TableRow
...
class MarkupDslTest {
    ...
    @Test
    fun tableBlockWrapsStandaloneTable() {
        val markup = article {
            section("S") {
        ...
        val tableBlock = (markup.blocks[0] as Section).blocks[0] as TableBlock
        assertEquals("My table", tableBlock.title)
        assertEquals("t1", tableBlock.id)
        assertNull(tableBlock.table.header)
        assertEquals(
            listOf(TableRow(listOf("a", "b")), TableRow(listOf("c", "d"))),
            tableBlock.table.rows,
        )
    }

    @Test
    fun tableBlockWithHeader() {
        val markup = article {
            section("S") {
                table {
                    header {
        ...
        val tableBlock = (markup.blocks[0] as Section).blocks[0] as TableBlock
        assertEquals(TableRow(listOf("col 1", "col 2")), tableBlock.table.header)
        assertEquals(1, tableBlock.table.rows.size)
    }
